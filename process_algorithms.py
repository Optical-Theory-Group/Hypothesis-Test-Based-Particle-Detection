...
def out_of_bounds_particle_penalty(theta, szx, szy, color_mode=None):
    ...
    if color_mode == 'gray' or color_mode == 'grayscale': 
        ...
        y_index = 4
        for i in range(1, len(theta)):
            x_term = position_penalty_function(theta[i,x_index], szx)
...
def jac_oob_penalty(theta, szx, szy, color_mode=None):
...
def hess_oob_penalty(theta, szx, szy, color_mode=None):
    ...
    if color_mode == 'gray' or color_mode == 'grayscale':
        ...
        d2dt2_oob_2d = np.zeros((len(theta) * 5 - 2, len(theta) * 5 - 2)) # Note: Only some of the diagonal terms are non-zero, because differentiating the penalty function with respect to two unrelated variables results in zero.
        ...
        for pidx in range(1, len(theta)):
            ...
            d2dt2_oob_2d[(pidx-1)*5 + 6][(pidx-1)*5 + 6] = d2dt2_position_penalty_function(theta[pidx, x_index], szx) * szx**2
            d2dt2_oob_2d[(pidx-1)*5 + 7][(pidx-1)*5 + 7] = d2dt2_position_penalty_function(theta[pidx, y_index], szy) * szy**2
...
def modified_neg_loglikelihood_fn(norm_flat_trimmed_theta, hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode=None):
...
def jacobian_fn(norm_flat_trimmed_theta, hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode=None):
    ...
    if color_mode == 'gray' or color_mode == 'grayscale': # If the image is a grayscale image, then roi_max and roi_min should be a single value.
        ...
        # These are independent of the particle intensity because it is a calculation based on a normalized gaussian function at the given position.
    ...
    if color_mode == 'gray' or color_mode == 'grayscale': # If the image is a grayscale image, then roi_max and roi_min should be a single value.
        ...
        one_minus_image_over_model_rgb = np.array([(1 - roi_image[:,:,ch] / roi_model[ch]) for ch in range(3)]) # This will be used many times in the following calculations. "1" will act as a matrix whose every element is one.
    ...
    ddt_oob = jac_oob_penalty(theta, szx, szy, color_mode=color_mode)  
...
def hessian_fn(norm_flat_trimmed_theta, hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode=None):
    ...
    d2dt2_nll_2d += hess_oob_penalty(theta, szx, szy, color_mode=color_mode) 
...
def get_tentative_peaks(image, min_distance=1,color_mode=None): # How does it perform for the case of RGB image? Not yet tested thoroughly (11/26/2024) [TODO]
...
def calculate_fisher_information_matrix_vectorized(norm_flat_trimmed_theta, szy, szx, hypothesis_index, minimum_model_value, psf_sigma, roi_max, alpha, color_mode=None):
    """
...
def generalized_maximum_likelihood_rule(roi_image, psf_sigma, last_h_index=5, random_seed=0, display_fit_results=False, display_xi_graph=False, use_exit_condition=False, roi_name=None, color_mode=None):
    ...
    alpha = (roi_max - roi_min) * 2 * np.pi * psf_sigma**2 # alpha.shape == (3,1) for RGB images, and alpha is a scalar for grayscale images.
    ...
    for hypothesis_index in range(last_h_index + 1): # hypothesis_index is also the number of particles. 
        ...
        # Only do the MLE (maximum likelihood estimation) for the hypothesis_index >= 1
            ...
            def callback_fn(xk, *args):
                ...
                hess = hessian_fn(xk, hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode)
            ...
            for minimizing_trial_index in range(num_minimize_trials):
                ...
                if record_snapshots:
                    ...
                    fn_snapshots.append(modified_neg_loglikelihood_fn(norm_flat_trimmed_theta, hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode))
                    hess_snapshots.append(np.nan)  # Initial Hessian is not available yet
                    theta_snapshots.append(norm_flat_trimmed_theta)
                    denormflat_theta_snapshots.append(denormalize(norm_flat_trimmed_theta, hypothesis_index, roi_max, szx, szy, alpha, color_mode).flatten())
                ...
                try:
                    with warnings.catch_warnings():
                        ...
                        if record_snapshots:
                            minimization_result = minimize(modified_neg_loglikelihood_fn, norm_flat_trimmed_theta, args=(hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode), method=method, jac=jacobian_fn, hess=hessian_fn, callback=callback_fn, options={'gtol': 100})
                        else:
                            minimization_result = minimize(modified_neg_loglikelihood_fn, norm_flat_trimmed_theta, args=(hypothesis_index, roi_image, roi_min, roi_max, minimum_model_value, psf_sigma, szx, szy, alpha, color_mode), method=method, jac=jacobian_fn, hess=hessian_fn, options={'gtol': 100})
        ...
        if hypothesis_index == 0:
            ...
            roi_model, _, _ = calculate_model_ipsfx_ipsfy(theta, range(szx), range(szy), hypothesis_index, minimum_model_value, psf_sigma, color_mode=color_mode)
        if color_mode == 'gray' or color_mode == 'grayscale':
